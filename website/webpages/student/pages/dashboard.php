...
// Helper: calculate normalized mark percentage for a student in a subject
...
$student_id_json = json_encode((string)$student_id);

// Get student's class info (class id and grade)
$sql_class = "
  SELECT c.id as class_id, c.grade 
  FROM class c 
  WHERE JSON_CONTAINS(c.students_json, '$student_id_json', '$.students') 
...
$class_rank = array_search($student_id, array_keys($class_students_marks)) + 1;
...
$grade_rank = array_search($student_id, array_keys($grade_students_marks)) + 1;

// --- CARD 3: DAYS ABSENT ---
$attendance_result = mysqli_query($conn, "SELECT attendance_json FROM academic_record WHERE student_id = $student_id AND school_year_id = $latest_year");
$attendance_json = [];
...
$marks_result = mysqli_query($conn, "SELECT marks_json FROM academic_record WHERE student_id = $student_id AND school_year_id = $latest_year");
$marks_json = null;
...
while ($year = mysqli_fetch_assoc($years_result)) {
    ...
    $res = mysqli_query($conn, "SELECT marks_json FROM academic_record WHERE student_id = $student_id AND school_year_id = $year_id");
    if ($row = mysqli_fetch_assoc($res)) {
        $marks_json = json_decode($row['marks_json'], true);
        $total_mark = 0; $count_subjects = 0;
        if (is_array($marks_json)) {
...
<div class="wrapper">
  ...
  <div class="content-wrapper" style="margin-top: 50px;">
    <section class="content-header">
      <div class="container-fluid"><h1>Student Dashboard</h1></div>
    </section>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="card card-danger">
              <div class="card-header"><h3 class="card-title">GPA Progress Over Years</h3></div>
              <div class="card-body">
          ...
          <div class="col-md-6">
            <div class="card card-success">
              <div class="card-header"><h3 class="card-title">Best Subjects This Year</h3></div>
              <div class="card-body">
